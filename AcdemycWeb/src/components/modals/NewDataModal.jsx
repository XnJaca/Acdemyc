...
export const NewDataModal = ({ columns = [], modalOpen = false, onClose, onSubmit }) => {
    ...
    const validationTextField = (objeto, name) => {

        const isValid = validateRequired(objeto[name]);
    ...
    const validarPrimeraEntrada = () => {
        ...
        for (const formValue of Object.keys(values)) {
            ...
            if (!validateRequired(values[formValue])) { 
                errors[formValue] = `${formValue} Es requerido`;
                //* Coloca el valor de la celda en el objeto de errores
    ...
    return (
        <Modal
            ...
            closeTimeOutMS={300}
        >
            <h1 className="text-center"> Agregar nuevo dato </h1>
            <hr />
